...
<body id="app-body">
    ...
    $(document).on('input', '.otp-box', function() {
        if ($(this).val() && $(this).next('.otp-box').length) $(this).next('.otp-box').focus();
        let fullOtp = "";
        $('.otp-box').each(function() { fullOtp += $(this).val(); });
        $('#input-otp').val(fullOtp);
    });

    $(document).on('keydown', '.otp-box', function(e) {
        if (e.key === 'Backspace' && !$(this).val() && $(this).prev('.otp-box').length) $(this).prev('.otp-box').focus();
    });
    ...
    $(document).on('submit', '#otp-send-form', function(e) {
        e.preventDefault();
        $('#send-otp-btn').prop('disabled', true).text('Verifying HR...');
        $.post("{{ route('auth.password.sendOtp') }}", $(this).serialize(), function(res) {
            if (res.success) switchStep(2); else alert(res.message);
    ...
    $(document).on('submit', '#otp-verify-form', function(e) {
        ...
        $.post("{{ route('auth.password.verifyOtp') }}", data, function(res) {
            if (res.success) switchStep(3); else alert(res.message);
        });
    });
    ...
    $(document).on('submit', '#password-reset-final-form', function(e) {
        e.preventDefault();
        let data = $(this).serialize() + "&email=" + $('#reset-email').val() + "&otp=" + $('#input-otp').val();
        $.post("{{ route('auth.password.update') }}", data, function(res) {
            if (res.success) {
                alert('HR Password Updated Successfully!');
                window.location.href = "{{ url('/hr-login') }}";
