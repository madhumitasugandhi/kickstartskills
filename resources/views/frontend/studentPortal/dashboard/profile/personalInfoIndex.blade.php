...
@section('icon', 'bi bi-pencil-square fs-4 p-2 bg-primary bg-opacity-10 rounded-3 text-primary')
        ...
        <div class="card-custom">
            <div class="d-flex flex-column flex-md-row align-items-center gap-4">
                ...
                <div class="position-relative" style="width: 100px;">
                    ...
                    <img src="{{ (isset($profile->profile_image) && $profile->profile_image) ? asset('uploads/student_profiles/' . $profile->profile_image) : 'https://ui-avatars.com/api/?name=' . urlencode($user->full_name) . '&background=4e73df&color=fff' }}"
                        class="profile-avatar-lg" id="student_img_preview"
                        style="width: 100px; height: 100px; border-radius: 50%; object-fit: cover; border: 3px solid var(--soft-blue);">

                    {{-- Camera Icon --}}
                    <div class="camera-icon-student" onclick="document.getElementById('student_image_input').click()"
                        style="position: absolute; bottom: 5px; right: 5px; background: #0d6efd; color: #fff; width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; border: 2px solid #1e293b; z-index: 10;">
                        <i class="bi bi-camera" style="font-size: 0.75rem;"></i>
                    </div>

                    {{-- Hidden File Input (Form ke bahar hai isliye form attribute zaroori hai) --}}
                    <input type="file" name="profile_image" id="student_image_input" form="profileForm" hidden
                ...
                <div class="text-center text-md-start flex-grow-1">
                    ...
                    <div
                        ...
                        <span><i class="bi bi-telephone me-1"></i> {{ $profile->phone ?? 'Not Provided' }}</span>
                    </div>

                    {{-- Tera Status Badge Logic --}}
                    @php
                        $status = strtolower($user->account_status ?? 'pending');
                        $bg = ($status == 'active') ? 'var(--soft-green)' : (($status == 'pending') ? 'var(--soft-orange)' : 'var(--soft-red)');
                        $color = ($status == 'active') ? 'var(--text-green)' : (($status == 'pending') ? 'var(--text-orange)' : 'var(--text-red)');
                    @endphp

                    <div class="status-badge d-inline-block px-3 py-1 rounded-pill"
                        style="background-color: {{ $bg }}; color: {{ $color }}; font-size: 0.75rem; font-weight: 600;">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            {{-- Agar users table mein hai toh $user->dob, agar profile mein hai toh $profile->dob --}}
                            <input type="date" name="dob" class="form-control profile-input"
                                value="{{ $user->dob ?? $profile->dob ?? '' }}" disabled>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <select name="blood_group" class="form-select profile-input" disabled>
                                <option value="" disabled {{ is_null($user->blood_group ?? $profile->blood_group ?? null) ? 'selected' : '' }}>Select Group</option>
                                @foreach(['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'] as $group)
                                    @php
                                        $currentValue = $user->blood_group ?? $profile->blood_group ?? '';
                                    @endphp
                                    <option value="{{ $group }}" {{ $currentValue == $group ? 'selected' : '' }}>{{ $group }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        ...
        // 🔥 CRITICAL FIX: Form submit hone se pehle inputs enable karna
        document.getElementById('profileForm').onsubmit = function () {
            const inputs = document.querySelectorAll('.profile-input');
            inputs.forEach(input => input.disabled = false);
            return true; // Form ko aage bhejne do
        };

        // Image preview logic (Already correct in your code)
        document.getElementById('student_image_input').addEventListener('change', function (e) {
            if (e.target.files && e.target.files[0]) {
                let reader = new FileReader();
                reader.onload = function (e) {
                    document.getElementById('student_img_preview').src = e.target.result;
                }
